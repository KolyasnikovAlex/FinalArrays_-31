...
int main() {
	...
	//	arr1[i] = rand() % (10 + 1 - 1) + 1; // можно 10+ 1
	//	std::cout << arr1[i] << ' ';
	//}
	//std::cout << std::endl;
	...
	//for (int day_namber = 0; day_namber < january_days; day_namber++) {
	//	january_temperature[day_namber] = rand() % (-5 + 1 -(-30)) + (-30);
	//	std::cout << january_temperature[day_namber] << ", ";
	//}
	//std::cout << "\b\b.\nВведите число  -> \n";
	//std::cin >> n;
	//int sum = 0; //для суммы темпертур января
	...
	//std::cout << "Количество дней, когда темпертару опускалась ниже " << n << ": " << counter << "\n\n";
	...
	for (int i = 0; i < size3; i++) {
		...
		//else
			//std::cout << i + 1 << ". " << arr3[i] << "\n";
		std::cout << i + 1 << ". " << arr3[i] << (arr3[i] >= i && arr3[i] <= m ? "!" : "") << '\n';
